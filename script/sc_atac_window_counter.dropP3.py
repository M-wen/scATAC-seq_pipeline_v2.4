import sys
import pysam
# Feb21, 2022. The separators in line41 and line55 were modified. raw output is "chr_start_end",now is "chr:start-end"
...
def counter(bedtuple,outsfile,first=False):
	...
	if includezeroes:
		for rec in bedtuple:
			recname = rec[0] + ":" + rec[1] + "-" + rec[2]
			currcounts = [0]*templen
			reads = bamfile.fetch(rec[0], int(eval(rec[1])), int(eval(rec[2])))
			for read in reads:
